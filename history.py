...
def history_page():
    ...
    if not activities:
        st.markdown("<h3 style='text-align: center'>Your history is empty 🗑️</h3>", unsafe_allow_html=True)
                ...
                for i, activity in enumerate(activities):
                    ...
                    with col1:
                        checked = st.checkbox("", key=i, value=st.session_state["all-checked"])
                        if checked:
                ...
                with cc2:
                    delete = st.button("Delete", disabled=not ids, use_container_width=True)
                    if delete:
